<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Yoshua Soto Dev | CV</title>
    <!--BOOTSTRAP CSS-->
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.6.1/dist/css/bootstrap.min.css" integrity="sha384-zCbKRCUGaJDkqS1kPbPd7TveP5iyJE0EjAuZQTgFLD2ylzuqKfdKlfG/eSrtxUkn" crossorigin="anonymous">
    <!--CUSTOM CSS-->
    <link rel="stylesheet" href="css/style.css">
    <!--GOOGLE FONTS-->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Moon+Dance&family=The+Nautigal&display=swap" rel="stylesheet">
</head>
<body>
    <!--NAV-->
    <section id="main">
      <nav class="navbar navbar-expand-lg navbar-light bg-light fixed-top">
        <div class="container">
          <a class="navbar-brand" href="#main">Yoshua Soto</a>
          <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
          </button>
          <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ml-auto">
              <li class="nav-item">
                <a class="nav-link" href="#main">Principal</a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="#about">Sobre Mi</a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="#education">Educación</a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="#intereses">Intereses</a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="#contact">Contacto</a>
              </li>
            </ul>
          </div>
        </div>
      </nav>
    </section>
    <!--HEADER-->
    <header id="header">
      <div class="container">
        <div class="row">
          <div class="cont">
            <h1 style="color: white; font-size: 55px;">Yoshua Soto | Desarrollador Web</h1>
            <br>
            <p style="color: white;" class="h4">Me llamo Yoshua Soto, soy Desarrollador Web y Programador BackEnd creativo e innovador, autodidacta, 
            entusiasta con las grandes ideas, avances del mundo tecnologíco y el mundo de los E-sports. 
            Conoce más sobre mi y lee sobre mis intereses.</p>
            <hr>
            <a href="#contact" class="btn btn-primary btn-lg" id="btn1">Contactame</a>
            <i class="fab fa-twitter"></i>
          </div>
        </div>
      </div>
    </header>
    <!--ABOUT-->
    <section id="about">
      <div class="container">
        <div class="row">
          <div class="col-md-6">
            <img src="img/photo.jpg" alt="" height="600" style="border-radius: 15%;" class="p-5">
          </div>
          <div class="col-md-6">
            <h1>Sobre mi</h1>
            <p>Soy un joven programador autodidacta, siempre estoy al pie del avance de la tecnología, 
              soy de pensamiento crítico, lógico y persuasivo, en mis ratos libres leo libros o escucho 
              los audiolibros con temática de economía, política, ciencia, programación, además de ciencia 
              ficción cyberpunk, distopia, y uno que otro libro sobre cultura japonesa, china y germánica. 
              Soy entusiasta de los E-Sports y del mundo de las criptomonedas, tradding y minado, creo 
              firmemente en el avance de la tecnología como muestra de la evolución de la mente humana, 
              comparto mucho la filosofía de Elon Musk al decir que; "Si hay algo que no va contra las leyes de la física, 
              puede lograrse".</p>
              <div class="row mt-5">
                <div class="col-md-6">
                  <h4>Información de Contacto</h4>
                  <p>Yoshua Soto
                    Trapichito Sector 3
                    Guarenas Edo. Miranda Venezuela
                    (+58) 0426-323-92-92
                    yoshuasoto54@gmail.com</p>
                </div>
                <div class="col-md-6 p-5">
                  <a href="pdf/yoshua-soto-2022.pdf" class="btn btn-outline-secondary btn-lg">Descarga mi CV</a>
                </div>
              </div>
          </div>
        </div>
      </div>
    </section>
    <!--EDUCATION-->
    <section id="education">
      <div class="container">
         <!--UNIVERSITY-->
        <div class="row mt-5">
          <div class="col-md-4">
            <h1 class="h2" style="padding: 20px;">Educación</h1>
          </div>
          <div class="col-md-8">
            <h1>Vida Universitaria</h1>
            <h3>IUT Rufino Blanco Fombona</h3>
            <p>TSU Informática • Agosto 2013 - Agosto 2016</p>
            <p>Carrera informática. Inicié en Agosto de 2013, con dudas pero con algo de seguridad en lo que estaba buscando, aprendí los fundamentos de la Informática, el análisis de sistemas y metodología de la investigación al realizar mi propia tesis de grado usando la metodología MEDSI, esta la aprové con la puntuación de 18/20 puntos, la misma la realicé solo, la universidad me enseño como realizar una investigación, análisis de datos, y como presentar los resultados de la investigación, fue una experiencia positiva y enriquecedora.</p>
          </div>
        </div>
        <hr>
         <!--WORKS-->
        <div class="row mt-5">
          <div class="col-md-4">
            <h1 class="h2" style="padding: 20px;">Trabajos</h1>
          </div>
          <div class="col-md-8">
            <h3>IUT Rufino Blanco Fombona</h3>
            <p>Supervisor • Marzo 2016 - Marzo 2018.</p>
            <p>Inicié haciendo pasantias en el unidad de Informática, donde reparaba equipos, hacia mantenimiento de los mismos, hice una topologia de red, luego me contrataron allí, inicié como Asistente de Sistemas junto con el Encargado del Dpto. de Informática, luego de unos cambios en la directiva del plantel, ascendí a Analista de Sistema y luego estuve a cargo del Dpto de Soporte Técnico como Supervisor del Dpto. estuve durante 2 años de servicio. Me encargaba del Help Desk, conexiones de red, Servidores locales, y todo lo referente al Soporte Técnico.</p>
          </div>
        </div>
        <div class="row mt-5">
          <div class="col-md-4">
            <h1 class="h2" style="padding: 20px;"></h1>
          </div>
          <div class="col-md-8">
            <h3>Multieliet Company</h3>
            <p>Programador BackEnd • Marzo 2018 - Octubre 2018.</p>
            <p>El dueño de la compañia me presentó la oferta para trabajar como programador BackEnd PHP usando los frameworks CodeIgniter y Laravel, fue un gran impulso para mi aprendizaje en el mmundo de la programación, aprendí a manejar herramientas como GitHub, duré poco tiempo ya que necesitaba mejorar en mis conocimientos y habilidades en el área del desarrollo web. En la compañia trabajaba de forma remota en Venezuela, ya que la compañia es Venezolana y dirigida por un familiar.</p>
          </div>
        </div>
        <hr>
        <!--SKILLS-->
        <div class="row mt-5 mb-5">
          <div class="col-md-4">
            <h1 class="h2" style="padding: 20px;">Habilidades</h1>
          </div>
          <div class="col-md-8">
            <p>Las habilidades fuertes son importantes, estas son las heramientas que utilizo cuando hago desarrollo web, muchas de las cuales he aprendido de forma autodidacta.</p>
            <div id="skills" class="mt-5">
              <div class="html">
                <p class="h4">HTML</p>  
                <div class="progress">
                    <div class="progress-bar bg-success" role="progressbar" style="width: 75%" aria-valuenow="75" aria-valuemin="0" aria-valuemax="100"></div>
                </div>
              </div>
              <br>
              <div class="css">
                <p class="h4">CSS</p>  
                <div class="progress">
                    <div class="progress-bar bg-success" role="progressbar" style="width: 65%" aria-valuenow="65" aria-valuemin="0" aria-valuemax="100"></div>
                </div>
              </div>
              <br>
              <div class="js">
                <p class="h4">JAVASCRIPT</p>  
                <div class="progress">
                    <div class="progress-bar bg-success" role="progressbar" style="width: 55%" aria-valuenow="55" aria-valuemin="0" aria-valuemax="100"></div>
                </div>
              </div>
              <br>
              <div class="php">
                <p class="h4">PHP</p>  
                <div class="progress">
                    <div class="progress-bar bg-success" role="progressbar" style="width: 65%" aria-valuenow="65" aria-valuemin="0" aria-valuemax="100"></div>
                </div>
              </div>
              <br>
              <div class="python">
                <p class="h4">PYTHON</p>  
                <div class="progress">
                    <div class="progress-bar bg-success" role="progressbar" style="width: 45%" aria-valuenow="45" aria-valuemin="0" aria-valuemax="100"></div>
                </div>
              </div>
              <br>
              <div class="bootstrap">
                <p class="h4">BOOTSTRAP</p>  
                <div class="progress">
                    <div class="progress-bar bg-success" role="progressbar" style="width: 68%" aria-valuenow="68" aria-valuemin="0" aria-valuemax="100"></div>
                </div>
              </div>
              <br>
              <div class="laravel">
                <p class="h4">LARAVEL</p>  
                <div class="progress">
                    <div class="progress-bar bg-success" role="progressbar" style="width: 60%" aria-valuenow="60" aria-valuemin="0" aria-valuemax="100"></div>
                </div>
              </div>
              <br>
              <div class="git">
                <p class="h4">GIT</p>  
                <div class="progress">
                    <div class="progress-bar bg-success" role="progressbar" style="width: 80%" aria-valuenow="80" aria-valuemin="0" aria-valuemax="100"></div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
    <!--INTERESES-->
    <section id="intereses">
      <div class="container">
        <h1 style="text-align: center;">Intereses</h1>
        <!--ECONOMY-->
        <div class="theme">
          <div class="row">
            <div class="col-md-4">
              <img src="img/portfolio/economia.jpg" alt="" height="300">
            </div>
            <div class="col-md-8 my-auto">
              <h2>Economia y Educación Financiera.</h2>
              <p class="mt-5">
                Para nuestra sociedad de Latinoamerica, estas ciencias esta siendo disponibles al alcance de todos, en la plataforma YouTube tenemos
                muchos tutores de estos temas que son parte de la vida cotidiana, las finanzas son parte importante para todo ambito de la vida, pero 
                he aprendido que no solo se trata del ahorro y de las inversiones, tambien son enseña a cuidar nuestros activos más preciados, la salud, 
                el tiempo, nuestro descanso, entender esto nos ha permitido tener una vida más optima ademas de entender como funciona la economia internacional, 
                como tener libertad financiera y ser abundantes, de esa manera podemos eliminar la mentalidad de escases y tener cultura financiera.
              </p>
            </div>
          </div>
        </div>
        <hr>
        <!--HISTORY-->
        <div class="theme">
          <div class="row">
            <div class="col-md-8">
              <h2>Historia Contemporanea.</h2>
              <p class="mt-5">
              La historia es tan importante como lo es el café en las mañanas, si no conocemos de la historia, estamos condenados a repetir los mismo 
              errores, esta actividad solitaria de leer sobre la historia, me ha permitido entender la evolución de las sociedades y la politica mundial 
              uno de los eventos más importantes que marcaron a la humanidad, fueron La Revolución Francesa, La revolucion Industrial, Los dos Guerras mundiales, 
              La Caida de la Unión Sovietica, y la Caida del Muro de Berlín, no son siemple actos de la humanidad, todos ellos dieron el inicio y el fin de la 
              renovación del ser humano, en los derechos individuales y en las libertades de cada individuo.
             </p>
              </p>
            </div>
            <div class="col-md 4">
              <img src="img/portfolio/historia.jpg" alt="" height="300">
            </div>
          </div>
        </div>
        <hr>
        <!--LITERATURA-->
        <div class="theme">
          <div class="row">
            <div class="col-md-4">
              <img src="img/portfolio/literatura.jpg" alt="" height="300">
            </div>
            <div class="col-md-8 my-auto">
              <h2>Literatura.</h2>
              <p class="mt-5">
                En una época donde tenemos al alcance tanta información e ideas muy útiles como los audiolibros, he tenido la oportunidad de sacar tiempo 
                muy valioso para dedicar a la lectura y escuchar audiolibros, muchos de los cuales captan mi interes, los percibo como un mensaje del pasado 
                que nos dejaron esos autores, plasmando sus conocimientos para cada uno de nosotros, estos siguen vigentes hoy dia, estas son mis obras literarias que 
                han marcado y mejorado mi vida: <br>
                El Arte de la Guerra. <br>
                Las 48 Leyes del Poder. <br>
                La Riqueza de las Naciones. <br>
                1984. <br>
                Ovejas Elécticas. <br>
                El Hombre en el Castillo. <br>
                Neuromante. <br>
                Yo, Robot. <br>
                La Via del Samurai. <br>
                Padre Rico, Padre Pobre.
              </p>
            </div>
          </div>
        </div>
        <hr>
        <!--PSICOLOGIA-->
        <div class="theme">
          <div class="row">
            <div class="col-md-8">
              <h2>Psicología.</h2>
              <p class="mt-5">
                Una de las ciencias de la salud que más me han interesado y ha permitido desarrollar mi potencial como persona ha sido la psicologia, estudios me 
                han ayudado a entender a las personas, conocerme a mi mismo y esto me permite conocer en donde debe trabajar como persona, conocer mis fortalezas y 
                a partir de allí, desarrollar mis habilidades en el campo laboral, los negocios, y las ideas. Gracias a internet, tenemos mucho más a nuestro alcance 
                esta ciencia de la salud para cuidar nuestra mente, ser positivos, proactivos y servir a la sociedad de una mejor manera.
              </p>
            </div>
            <div class="col-md 4">
              <img src="img/portfolio/psicologia.jpg" alt="" height="300">
            </div>
          </div>
        </div>
        <hr>
        <!--MUSICA-->
        <div class="theme">
          <div class="row">
            <div class="col-md-4">
              <img src="img/portfolio/musical.jpg" alt="" height="300">
            </div>
            <div class="col-md-8 my-auto">
              <h2>Producción Musical.</h2>
              <p class="mt-5">
                Desde muy pequeño, me ha interesado la música, soy seguidor de muchas bandas y artistas sobre todo de Alemania y Japón, así que decidí de forma individual 
                aprender a crear música usando el software LMMS, tambien tengo gustó por tocar la guitarra, la bateria y cantar, la música es un verdadero arte que 
                ha pasado por generaciones, y doy mi afirmación que actualmente hay mucha gente que hace muy buena música, pero no es la que esta de moda, es interesante conocer como las 
                influencias musicales han permitido que las nuevas generaciones creen fusiones y permitan la evolución de los géneros musicales.  
              </p>
            </div>
          </div>
        </div>
    </section>
  <!--CONTACT-->
    <section id="contact">
      <div class="container">
        <div class="row">
          <div class="col-md-2">
            <div class="icon-email">
              <i class="bi bi-envelope-fill"></i>
              <svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" fill="currentColor" class="bi bi-envelope-fill" viewBox="0 0 16 16">
                <path d="M.05 3.555A2 2 0 0 1 2 2h12a2 2 0 0 1 1.95 1.555L8 8.414.05 3.555ZM0 4.697v7.104l5.803-3.558L0 4.697ZM6.761 8.83l-6.57 4.027A2 2 0 0 0 2 14h12a2 2 0 0 0 1.808-1.144l-6.57-4.027L8 9.586l-1.239-.757Zm3.436-.586L16 11.801V4.697l-5.803 3.546Z"/>
              </svg>
            </div>
          </div>
          <div class="col-md-10">
            <h4 class="py-3">¿Necesitas una consulta?, ¿Desea saber más o contratar mis servicios? Escríbeme a la direccion que aparece abajo y contácteme.</h4>
          </div>
          <div class="row">
            <div class="col-md-7 mt-5">
              <h2>Twitter</h2>
              <p class="mt-5">
                <a class="twitter-timeline" data-width="600" data-height="400" data-theme="dark" href="https://twitter.com/KloYoshi?ref_src=twsrc%5Etfw">Tweets by KloYoshi</a> <script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>
              </p>
            </div>
            <div class="col-md-5 p-5 my-auto">
              <h3>Dirección y Teléfonos</h3>
              <p>Yoshua Soto</p>
              <p>Guarenas Edo. Miranda Venezuela</p>
              <p>yoshuasoto54@gmail.com</p>
              <p>(+58) 0426-323-9292</p>
            </div>
          </div>
        </div>
      </div>
    </section>
    <!--FOOTER-->
    <section id="footer">
      <div class="container">
        <div class="list-icon">
          <!--TWITTER-->
            <a href="https://twitter.com/KloYoshi"> 
              <i class="bi bi-twitter"></i>
              <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" class="bi bi-twitter" viewBox="0 0 16 16">
                <path d="M5.026 15c6.038 0 9.341-5.003 9.341-9.334 0-.14 0-.282-.006-.422A6.685 6.685 0 0 0 16 3.542a6.658 6.658 0 0 1-1.889.518 3.301 3.301 0 0 0 1.447-1.817 6.533 6.533 0 0 1-2.087.793A3.286 3.286 0 0 0 7.875 6.03a9.325 9.325 0 0 1-6.767-3.429 3.289 3.289 0 0 0 1.018 4.382A3.323 3.323 0 0 1 .64 6.575v.045a3.288 3.288 0 0 0 2.632 3.218 3.203 3.203 0 0 1-.865.115 3.23 3.23 0 0 1-.614-.057 3.283 3.283 0 0 0 3.067 2.277A6.588 6.588 0 0 1 .78 13.58a6.32 6.32 0 0 1-.78-.045A9.344 9.344 0 0 0 5.026 15z"/>
              </svg>
            </a>
          <!--FACEBOOK-->
            <a href="https://www.facebook.com/profile.php?id=100076023571851">
              <i class="bi bi-facebook"></i>
              <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" class="bi bi-facebook" viewBox="0 0 16 16">
                <path d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951z"/>
              </svg>
            </a>
          <!--GITHUB-->
            <a href="https://github.com/fallserkerg1?tab=repositories">
              <i class="bi bi-github"></i>
              <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" class="bi bi-github" viewBox="0 0 16 16">
                <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z"/>
              </svg>
            </a>
          <!--LINKEDIN-->
            <a href="https://www.linkedin.com/in/yoshua-soto-oliveros-025818160/">
              <i class="bi bi-linkedin"></i>
              <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" class="bi bi-linkedin" viewBox="0 0 16 16">
                <path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854V1.146zm4.943 12.248V6.169H2.542v7.225h2.401zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248-.822 0-1.359.54-1.359 1.248 0 .694.521 1.248 1.327 1.248h.016zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016a5.54 5.54 0 0 1 .016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225h2.4z"/>
              </svg>
            </a>
        </div>
          <p style="text-align: center; color: gray;">Copyright 2022 © | Yoshua Soto</p>
      </div>
    </section>
</body>
    <!--BOOTSTRAP JS-->
    <script src="https://cdn.jsdelivr.net/npm/jquery@3.5.1/dist/jquery.slim.min.js" integrity="sha384-DfXdz2htPH0lsSSs5nCTpuj/zy4C+OGpamoFVy38MVBnE+IbbVYUew+OrCXaRkfj" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@4.6.1/dist/js/bootstrap.bundle.min.js" integrity="sha384-fQybjgWLrvvRgtW6bFlB7jaZrFsaBXjsOMm/tB9LTS58ONXgqbR9W8oWht/amnpF" crossorigin="anonymous"></script>
    <script>
      //smooth scrolling
      document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            e.preventDefault();
    
            document.querySelector(this.getAttribute('href')).scrollIntoView({
                behavior: 'smooth'
            });
        });
    });
    </script>
</html>